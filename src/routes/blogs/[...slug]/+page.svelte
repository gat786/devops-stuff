<script lang="ts">
    ...
    onMount(() => {
        addEventListener("scroll", (event) => {
            ...
            if (scrollPercent > 90 && $canShowToast) {
                toastComponent.invokeShowTooltip({
                    ...
                    _message:
                        "Thank you for reading my blog this far, Hope you enjoyed it. Please leave a comment if you have any feedback.",
                });
        ...
        if (host.includes(localhost)) {
            console.log(
                "running on localhost, adding a pageview would be cheating.",
            );
            return;
        } else {
...
<div class="flex flex-col items-center w-screen">
    ...
    <article
        id="blog-content"
        class="prose dark:prose-invert mt-4 mx-8 w-10/12"
    >
        {@html data.content}
    </article>
    ...
    <div class="my-8 text-2xl font-thin text-center">
        ...
        Please leave a comment if you have any feedback. <br />
    </div>

    <Comments
        website-id={11051}
        page-id={`blogs/${data.front_matter.url_postfix}`}
    />
    <div class="fixed bottom-10 left-10 z-10"></div>
